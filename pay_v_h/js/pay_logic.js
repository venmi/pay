...
$.switchTab = function (index) {
			...
			$.pageGuideItemClick($(".page_guide_item_text>a")[index]);
...
$(document).ready(function(){		
		...
		$.each(pay_params['webpay_paytype_url'],function(n,value) {
			...
			var tabItemTpl1 = '<a class="c_btn click_btn" id = "{0}" style="{1}: 3px solid #2196f3; color: #2196f3;"><span><img src="images/{2}_coin_after.png" click="clicked"/></span><p>{3}</p></a>';
			var tabItemTpl2 = '<a class="c_btn" id="{0}"><span><img src="images/{1}_coin.png"/></span><p>{2}</p></a>';
		...
		var defaultPayKey = $.getPayKey(pay_params['webpay_paytype_url'], 0);

		if (defaultPayKey != '') {
			$('#content_' + defaultPayKey).css('display', 'block');
		}
		...
		$('.product_description').text(pay_params.product_description);
...
//		})
				...
				success:function(data) {
					...
					if (data.ret == 0) {
						...
						if (retryWaitingCount < tuitui.sdk.config.pay_waiting_retry) {
							...
							TTAndroid.closeLoadingDialog();
							$.showHintDialog(tuitui.sdk.local.pay_waiting_timeout, tuitui.sdk.HINT_CONFIRMATION_DIALOG);
						}
						
					} else if (data.ret == -19045) {
								$.showHintDialog(data.data.pay_msg, tuitui.sdk.HINT_CLOSE_DIALOG);
					} 
					else {
						TTAndroid.closeLoadingDialog();
						$.showHintDialog(data.msg, tuitui.sdk.HINT_DIALOG);
					}
				},
				error:function(er){
					TTAndroid.closeLoadingDialog();
					$.showHintDialog(er, tuitui.sdk.HINT_CLOSE_DIALOG);
				}
			});
		}
		...
		if ($('#do_pay_shenzhoufu').length != 0) {
			...
			$('#do_pay_shenzhoufu').click(function(){
				...
				if (pay_params['webpay_paytype_url'][req_params.select_index]['card_money'] == '0') {
					$.showToast(tuitui.sdk.local.empty_card_money);
					return;
				} else if (sn == tuitui.sdk.local.empty_sn || sn.length == 0) {
					$.showToast(tuitui.sdk.local.empty_sn);
					return;
				} else if (password == tuitui.sdk.local.empty_password || password.length == 0) {
					$.showToast(tuitui.sdk.local.empty_password);
					return;
				} 
				...
				if (rechargeCardType == "") {
					$.showToast(tuitui.sdk.local.recharge_card_format);
					return;
				}
				...
				$.ajax({
					...
					success:function(data) {
						if (data.ret == 0 && data.data.pay_ret == 0) {
							...
							if (data.ret == 0) {
								$.showHintDialog(data.data.pay_msg, tuitui.sdk.HINT_DIALOG);
							}else {
								$.showHintDialog(data.msg, tuitui.sdk.HINT_DIALOG);
							}
						}
		...
		$('#get_user_currency').click(function(){
			var index = req_params.select_index;
			/*
			if ($($('.footerimg')[index]).hasClass('hide')) {
				$($('.footerimg')[2]).attr('class', 'footerimg');
			} else {
				$($('.footerimg')[2]).attr('class', 'footerimg hide');
			}*/
			...
			$.ajax({
				...
				success:function(data) {
					...
					if (data.ret == 0) {
						...
						$('.balance').text(' ' + pay_params.balance/100 + ' ');
						TTAndroid.notifyRefreshBalance(balance);
